...
#include <linux/kpd.h>
#include <mach/eint.h>   //add by elink yq@20140828
...
int had_read = 0, hotkeyIdx=-1, kpd_pressed, send_keycode=1;
#define TKEY_RELEASE 0
#define TKEY_PRESS   1
u16 tkey_state=TKEY_RELEASE;
...
/* wade, for Function Key */
static struct task_struct *funckey_task = NULL;
int tkey_kthread(void *arg);
extern u16 kpd_func_keymap[];
extern int tca6416_read_reg(int reg, u16 *val);
...
#if defined(HALL_USER_EINT_CONFIG)   ////Elink_Paul add @20140313
#define KPD_HALL_USER_EINT       1

#if defined(KPD_HALL_USER_EINT)

//chg by elink yq@20140828
...
void kpd_hall_eint_handler_event(unsigned long data){
	...
	mt_eint_mask(CUST_EINT_MHALL_NUM);  //chg by elink yq@20140828
...
//	printk("\n HALL \n ON GPIO status __PAUl__ %d\n",mt_get_gpio_in(GPIO_GSE_2_EINT_PIN));
	...
	{  
	   ...
	   if(!lcd_hall_status)
		...
		//ELINK Qing.Yan modify for HALL FALLING AND RISING TRIGER.
	...
	kpd_print("%s() : report Linux keycode = %u\n", __func__, KPD_PWRKEY_MAP);

	mt_eint_unmask(CUST_EINT_MHALL_NUM);  //chg by elink yq@20140828
...
#endif ////Elink_Paul add @20140313 end
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			if (hw_keycode == 36) hotkeyIdx = 1;
			else if (hw_keycode == 18) hotkeyIdx = 2;
			else if (hw_keycode == 9) hotkeyIdx = 3;
			else if (hw_keycode == 0) hotkeyIdx = 4;
			else continue;
			linux_keycode = kpd_func_keymap[hotkeyIdx];
			...
			if (send_keycode) {
			  ...
			  input_sync(kpd_input_dev);
			  IMT8_printk("[wade] Key %d = %d (%s)\n", hotkeyIdx, linux_keycode, (pressed?"pressed":"release"));
                        }
                        else
			  IMT8_printk("[wade] DISABLED Key %d = %d (%s)\n", hotkeyIdx, linux_keycode, (pressed?"pressed":"release"));
		}
	}
...
int tkey_kthread(void *arg)
{
  ...
  while (!kthread_should_stop()) {
    ...
    schedule_timeout(HZ/10); // sleep for 1sec
    if (0 == tca6416_read_reg(TCA6416_OUTPUT, &val)) {
      tkey = val & 0x00FF;
      if (tkey == 0x00FC) { // press
        if (tkey_state == TKEY_RELEASE) {
	  hotkeyIdx = 0;
	  if (send_keycode && (kpd_func_keymap[hotkeyIdx] > 0)) {
            input_report_key(kpd_input_dev, kpd_func_keymap[hotkeyIdx], 1);
            input_sync(kpd_input_dev);
	  }
          tkey_state = TKEY_PRESS;
        }
      }
      else if (tkey == 0xFD) { // release
        if (tkey_state == TKEY_PRESS) {
	  hotkeyIdx = 0;
	  if (send_keycode && (kpd_func_keymap[hotkeyIdx] > 0)) {
            input_report_key(kpd_input_dev, kpd_func_keymap[hotkeyIdx], 0);
            input_sync(kpd_input_dev);
	  }
          tkey_state = TKEY_RELEASE;
        }
      }
    }
  }
...
static int __init kpd_mod_init(void)
{
	...
	// for Function Key
	r = 0;
	funckey_task = kthread_create(tkey_kthread, NULL, "tkey_kthread");
	if (IS_ERR(funckey_task)) {
	  r = PTR_ERR(funckey_task);
	  funckey_task = NULL;
	}
	else {
	  wake_up_process(funckey_task);
	}
