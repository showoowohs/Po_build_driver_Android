...
#define WADE 1 // 1 is normal mode
#if WADE == 1
#include <linux/bq20z75.h>
extern int procfs_get_gpio(int gpio);
#endif
...
#if WADE == 1
        if (bq20z75_set_cmd_read(BQ20z75_CMD_RelativeStateOfCharge, &(val->intval)) < 0) {
          val->intval = last_capacity;
          battery_status=0;
        }
        else {
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    {     
		...
		// added by phil @20140102 begin
        if (BMT_status.bat_vol <= SYSTEM_OFF_VOLTAGE && gFG_Is_Charging == KAL_FALSE)
        {
            printk("phil debug ready to shutdown system!!\n", gFG_Is_Charging);
            resetBatteryMeter = mt_battery_0Percent_tracking_check();
        }
		// added end
    }
...
static void mt_battery_notify_VCharger_check(void)
{
	...
	if(BMT_status.charger_vol >= V_CHARGER_MAX && BMT_status.charger_vol <= CHARGER_OVP_MAX)	// Elink_phil added @20140215 for low charger vol warning
    {
...
// Elink_phil added @20140215 for low charger vol warning begin
#if (V_CHARGER_ENABLE == 1)
	if(BMT_status.charger_vol <= V_CHARGER_MIN && BMT_status.charger_vol >= CHARGER_OVP_MIN)
    {
        g_BatteryNotifyCode |= 0x0020;
...
static void mt_battery_update_status(void)
{
...
#else
	{
		...
		ac_update(&ac_main);
		usb_update(&usb_main);
        battery_update(&battery_main);          // modify by phil @20140102
	}
