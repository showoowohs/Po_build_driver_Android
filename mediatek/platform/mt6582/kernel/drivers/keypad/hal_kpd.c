...
u16 kpd_keymap[KPD_NUM_KEYS] = KPD_INIT_KEYMAP();
u16 kpd_func_keymap[5];
int had_init_kpd_func_keymap = 0;
...
void imobile_init_kpd_func_keymap(u16 k1, u16 k2, u16 k3, u16 k4, u16 k5)
{
  int i;
  had_init_kpd_func_keymap = 1;
  kpd_func_keymap[0] = k1;
  kpd_func_keymap[1] = k2;
  kpd_func_keymap[2] = k3;
  kpd_func_keymap[3] = k4;
  kpd_func_keymap[4] = k5;
  IMT8_printk("[kpd] remap %d %d %d %d %d\n",
    kpd_func_keymap[0], kpd_func_keymap[1], kpd_func_keymap[2],
    kpd_func_keymap[3], kpd_func_keymap[4]);
  for (i = 0; i < 5; i++) {
    if (kpd_func_keymap[i] != 0)
      __set_bit(kpd_func_keymap[i], kpd_input_dev->keybit);
  }
}
...
void kpd_pmic_rstkey_hal(unsigned long pressed){
	...
	if(!kpd_sb_enable){
                kpd_pressed = pressed;
                had_read = 0;
		if (!had_init_kpd_func_keymap)
			imobile_init_kpd_func_keymap(0, KEY_HOME, KEY_BACK, KEY_VOLUMEUP, KEY_VOLUMEDOWN);

                hotkeyIdx = 1;
		if (send_keycode) {
		  input_report_key(kpd_input_dev, kpd_func_keymap[hotkeyIdx], kpd_pressed);
		  input_sync(kpd_input_dev);
		}
		if (kpd_show_hw_keycode) {
			IMT8_printk("[wade] KEY-1 = %u (%s)\n",
			  kpd_func_keymap[1], (kpd_pressed?"pressed":"release"));
		}
	}
#endif
}
...
void kpd_pmic_pwrkey_hal(unsigned long pressed){
	...
	if(!kpd_sb_enable){
		...
		input_sync(kpd_input_dev);
		if (kpd_show_hw_keycode) {
			IMT8_printk(KPD_SAY "(%s) HW keycode =%d using PMIC\n",
...
void kpd_pwrkey_handler_hal(unsigned long data){
	...
	input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, pressed);
	input_sync(kpd_input_dev);
	IMT8_printk("power key by EINT, report Linux keycode = %u\n", KPD_PWRKEY_MAP);
